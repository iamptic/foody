# main.py — Foody Backend v5 (offers/reservations + merchant CRUD + admin + debug)
import os, time
...
__version__ = "foody-backend-v5-20250812011423"
...
async def _debug_ping_db():
    ...
    return {"ok": True, "ms": dt_ms}

@app.get("/debug/check_restaurant")
async def _debug_check_restaurant(rid: str):
    async with _engine_admin.connect() as conn:
        row = (await conn.execute(text("SELECT id, api_key FROM foody_restaurants WHERE id=:rid").bindparams(rid=rid))).first()
    if not row:
